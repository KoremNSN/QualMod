function MDM_PTB_med1_v2(observer);
...
% Aging version:
%       - 10s max display + 0.5s feedback + 4.5s ITI = max 15s
...
Datamed.stimulus.lottoDisplayDur=10;
Datamed.stimulus.ITIDur = 4.5; %for debug, time = 0.5; for the actual task, time = 4.5
...
blockNum=1; % first block of both monetary and medical
if isfile(['MDM_MON_' observer])
...
blockNum=2; % first block of both monetary and medical
breakscript = '\n\nYou can take a short break now.\nPlease remain seated and do not move your chair.\n\n[When ready, press 5 to start]';
        % introduce a pause 
...
%        eyetracker.stop_gaze_data();
        ...
        Screen(Win.win, 'FillRect',1);
        Screen('TextSize', Win.win, Datamed.stimulus.fontSize.pause);
        DrawFormattedText(Win.win,'Finished the first two Medical Blocks.\nSaving data...','center','center',Datamed.stimulus.fontColor);
        Screen('flip',Win.win);
        eval(sprintf('save %s.mat Datamed',Datamed.filename)) % save data
        WaitSecs(1);
        %waitForBackTick;
