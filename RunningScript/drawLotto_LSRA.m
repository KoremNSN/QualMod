...
%%% added eye tracking here
Data.trialTime(trial).TobiiTrialStartTime= Tobii.get_system_time_stamp;
...
while elapsedTime<Data.stimulus.lottoDisplayDur % while the 10 seconds max is not reached
    ...
    if keyisdown && (keyCode(KbName('/')) || keyCode(KbName('*'))) % 1 or 2 is pressed  1! or 2@
...
if keyisdown && keyCode(KbName('/'))            %% Reversed keys '1' and '2' - DE % 1! 
    Data.choice(trial)=1;
...
elseif keyisdown && keyCode(KbName('*'))        %% Reversed keys '2' and '1' - DE  2@
    Data.choice(trial)=2;
    Data.rt(trial)=elapsedTime;
    if Data.refSide==2
...
Data.gazeAll{trial} = eyetracker.get_gaze_data();
Data.trialTime(trial).TobiiTrialEndTime= Tobii.get_system_time_stamp;
